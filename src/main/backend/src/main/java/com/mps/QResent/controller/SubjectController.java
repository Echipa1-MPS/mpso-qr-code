...
public class SubjectController {
    ...
    public String getAllCourses(){
        ...
        for(SubjectView subject: subjects){
            ...
            jsonObject1.put("Id_Professor", userService.getProfId(subjectService.findById(subject.getId()).get()));
            jsonObject1.put("Professor", userService.getProf(subjectService.findById(subject.getId()).get()));
    ...
    public Long createSubject(@RequestBody SubjectDTO subjectDTO) {
        ...
        if(userService.findById(subjectDTO.getIdProfessor()).isPresent()){
            subject.getUsers().add(userService.findById(subjectDTO.getIdProfessor()).get());
            subjectService.save(subject);
        }
    ...
    public void updateSubject(@RequestBody SubjectDTOUpdate subjectDTO){
        ...
        if(subject.isPresent()){
            ...
            if(subjectDTO.getIdProfessor()!= null){
                Optional<User> user = userService.findById(subjectDTO.getId());
    ...
    public void enrollStudents(@RequestBody StudentsEnroll studentsEnroll){
        ...
        if(subject.isPresent()){
            for (StudentsToEnroll student: studentsEnroll.getStudents_to_enroll()){
                Optional<User> user = userService.findById(student.getId_user());
                if(user.isPresent()){
